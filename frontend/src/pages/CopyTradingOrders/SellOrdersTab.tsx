...
import type { SellOrderInfo, OrderTrackingRequest, AccountOrderTrackingRequest, OrderTrackingListResponse, MarketGroupedOrdersRequest, MarketGroupedOrdersResponse, MarketOrderGroup, BridgeTradeRecord, BridgeTradeRecordListResponse } from '../../types'
...
const SellOrdersTab: React.FC<SellOrdersTabProps> = ({ copyTradingId, accountId, active = false }) => {
  ...
  useEffect(() => {
    if ((copyTradingId || accountId) && active) {
      fetchOrders()
    }
  }, [copyTradingId, accountId, active, page, limit, filters.marketId, filters.status, groupByMarket])

  useEffect(() => {
    if (copyTradingId && active) {
      fetchBridgeRecords()
    }
  }, [copyTradingId, active, filters.marketId, filters.marketTitle, filters.status])
  ...
  const fetchOrders = async () => {
    if (!copyTradingId && !accountId) return

    setLoading(true)
    try {
      if (groupByMarket && !isAccountMode) {
        // 调用分组接口
        ...
        let response
        if (isAccountMode) {
          const request: AccountOrderTrackingRequest = {
          ...
          response = await apiService.accounts.orders(request)
        } else {
          ...
          response = await apiService.orderTracking.list(request)
        }
        if (response.data.code === 0 && response.data.data) {
  ...
  const fetchBridgeRecords = async () => {
    ...
    try {
      ...
      if (response.data.code === 0 && response.data.data) {
        ...
        const sellRecords = (data.list || []).filter(record => {
          const marketIdMatched = !filters.marketId || record.marketId?.toLowerCase().includes(filters.marketId.toLowerCase())
          const marketTitleMatched = !filters.marketTitle || record.marketTitle?.toLowerCase().includes(filters.marketTitle.toLowerCase())
          const statusMatched = !filters.status || record.status === filters.status
          return record.side === 'SELL' && marketIdMatched && marketTitleMatched && statusMatched
        })
        setBridgeRecords(sellRecords)
        setBridgeTotal(sellRecords.length)
      }
    } catch (error: any) {
      console.error('获取 Bridge 卖出执行记录失败:', error)
    } finally {
  ...
  const bridgeColumns = [
    ...
    {
      title: t('copyTradingOrders.errorMessage') || '失败原因',
      dataIndex: 'errorMessage',
      key: 'errorMessage',
      width: 180,
      render: (errorMessage: string | undefined) => errorMessage || '-'
    },
    {
    ...
    <div>
      <div style={{ marginBottom: 16, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        >
          ...
          <Option value="SUCCESS">Bridge 成功</Option>
          <Option value="FAILED">Bridge 失败</Option>
          <Option value="PENDING">Bridge Pending</Option>
        </Select>
      ...
      {(bridgeTotal > 0 || bridgeLoading) && (
        <div style={{ marginTop: 32 }}>
          <Divider orientation="left">{t('copyTradingOrders.bridgeSellExecutions') || 'Bridge 卖出执行记录'}</Divider>
          <Table
