...
export const getPolymarketUrl = (
  ...
  eventSlug?: string | null,  // 跳转用的 slug（从 events[0].slug 获取）
  _marketCategory?: string | null,  // 保留参数以便未来使用
  marketId?: string | null,
  isMoneyline?: boolean
): string | null => {
  // 优先使用 marketSlug 跳转到具体市场页；没有则使用 eventSlug 跳转到事件页
  if (marketSlug) {
  ...
  if (eventSlug) {
    ...
    if (isMoneyline === true) {
      return `https://polymarket.com/event/${eventSlug}/moneyline`
    }
